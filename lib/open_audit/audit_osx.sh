...
system_pc_date_os_installation=`/bin/date -r $(/usr/bin/stat -f "%B" /private/var/db/.AppleSetupDone) "+%Y-%m-%d %H:%M:%S"`
if [[ "$system_model" == *"MacBook"* ]]; then
	system_form_factor="laptop"
fi


xml_file="$system_hostname"-`/bin/date +%Y%m%d%H%M%S`.xml
...
processor_speed=`sudo system_profiler SPHardwareDataType | grep "Processor Speed:" | cut -d":" -f2 | sed 's/^ *//g' | cut -d" " -f1 | /usr/bin/tr ',' '.'`
processor_speed=`echo "scale = 0; $processor_speed * 1000" | /usr/bin/bc`
